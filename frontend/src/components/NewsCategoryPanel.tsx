...
const CATEGORY_META: Record<string, { icon: string; labelKey: string; color: string }> = {
  market:       { icon: '📈', labelKey: 'category.marketImpact', color: '#667eea' },
  policy:       { icon: '🏛️', labelKey: 'category.policyImpact', color: '#f59e0b' },
  earnings:     { icon: '💰', labelKey: 'category.earnings', color: '#10b981' },
  product_tech: { icon: '🚀', labelKey: 'category.productTech', color: '#8b5cf6' },
    ...
    <div className="news-category-wrap">
      <div className="news-category-bar">
        {keys.map((key) => {
          ...
          const meta = CATEGORY_META[key] || { icon: '📌', labelKey: '', color: '#667eea' };
          ...
          return (
            ...
            >
              ...
              <div className="category-tag-body">
                <span className="category-tag-label">{meta.labelKey ? t(meta.labelKey) : key}</span>
      ...
      {activeCat && (
        <div className="sentiment-sub-bar">
          ...
          >
            {t('category.allSentiment')} <span className="sentiment-sub-count">{activeCat.count}</span>
          </button>
          <button
