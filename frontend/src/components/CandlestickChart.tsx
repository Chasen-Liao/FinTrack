...
export default function CandlestickChart({ symbol, refreshToken = 0, lockedNewsId, highlightedArticleIds, highlightColor, zoomRange, onZoomReset, onHover, onRangeSelect, onArticleSelect, onDayClick }: Props) {
  ...
  function drawChart(rawData: OHLCRow[], particles: Particle[]) {
    ...
    const greenGrad = defs.append('linearGradient')
      .attr('id', 'candle-green')
      .attr('x1', '0%').attr('y1', '0%')
      .attr('x2', '0%').attr('y2', '100%');
    greenGrad.append('stop').attr('offset', '0%').attr('stop-color', '#22c55e');
    greenGrad.append('stop').attr('offset', '100%').attr('stop-color', '#16a34a');
    ...
    const redGrad = defs.append('linearGradient')
      ...
      .attr('x1', '0%').attr('y1', '0%')
      .attr('x2', '0%').attr('y2', '100%');
    redGrad.append('stop').attr('offset', '0%').attr('stop-color', '#ef4444');
    redGrad.append('stop').attr('offset', '100%').attr('stop-color', '#dc2626');
    ...
    // Bodies with gradient fill
    candles.append('rect')
      .attr('x', (d) => x(d.date) - candleWidth / 2)
      .attr('y', (d) => y(Math.max(d.open, d.close)))
      .attr('width', candleWidth)
      .attr('height', (d) => Math.max(1, Math.abs(y(d.open) - y(d.close))))
      .attr('fill', (d) => (d.close >= d.open ? 'url(#candle-green)' : 'url(#candle-red)'))
