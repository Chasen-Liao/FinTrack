  ...
  return (
    <div className="news-panel">
      <div className="news-panel-header">
        ...
        <button className="range-clear-btn" onClick={onClose}>{t('range.close')}</button>
      </div>

      <div className="range-news-dates">
        {startDate} ~ {endDate}
        {data && <span className="news-count" style={{ marginLeft: 8 }}>{data.total} {t('news.articles')}</span>}
      </div>
      ...
      {loading ? (
        ...
        <div className="news-empty">{t('news.noNews')}</div>
      ) : (
        <div className="news-list">
          ...
          {data.top_bullish.length > 0 && (
            <div className="range-news-section">
              <div className="range-news-section-title bullish">
                ▲ {t('rangeNews.bullish')} ({data.top_bullish.length})
              </div>
          ...
          {data.top_bearish.length > 0 && (
            <div className="range-news-section">
              <div className="range-news-section-title bearish">
                ▼ {t('rangeNews.bearish')} ({data.top_bearish.length})
              </div>
          ...
          {data.articles.length > 0 && (
            <div className="range-news-all">
              ...
              >
                {showAll ? t('rangeNews.hideAll') : t('rangeNews.showAll')} {data.total} {t('news.articles')}
          ...
          <button
            ...
            onClick={() => onAskAI(t('range.whatsDriving'))}
          >
            {t('range.askPokieTicker')}
          </button>
        </div>
      )}
    </div>
